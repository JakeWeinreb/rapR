...
The first thing I wanted to check is how the use of money words changed over time.  Were older artists more/less likely to brag about their cash?
...
How many songs an artist has with the word money in it
```{r}
artists_clean %>% filter(artist_name == "Gucci Mane") %>% .$track_name %>% unique() %>% tolower() %>% str_count("money") %>% sum()
```
